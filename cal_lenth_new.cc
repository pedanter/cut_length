...
bool compareColumn(const std::vector<int>& a, const std::vector<int>& b) {
...
/*
void n_for_loop(std::vector<int>max_num, std::vector<double>length_vec, std::vector<double>& residue_vec, std::vector<std::vector<int>>& tt_vec, int size, double length, int num) {
	// 定义一个数组，存储每层循环的变量
	int* loop_vars = new int[size];
	// 初始化每个变量为0
	...
	while (level < size) {
		...
		loop_vars[level]++;  // 对当前层的变量加一

		for (level = 0; level < size && loop_vars[level] >= size; level++) {
			// 如果当前层的变量达到了n，就将其置为0，然后进入下一层
			loop_vars[level] = 0;
		}
	}
	// 释放数组的内存
...

{
	...
	while (level < num ) {
		...
		//int tm_num = 0;
		for (int i = 0; i < num; i++) {
			tm_residue = tm_residue - loop_vars[i] * length_vec[i];
			//tm_num = tm_num + loop_vars[i];
		...
		if (tm_residue >= 0) {
			residue_vec.push_back(tm_residue);
			for (int i = 0; i < num; i++) {
				tn_vec[i].push_back(loop_vars[i]);
		...
		*/
		...
		for (level = 0; level <num  && loop_vars[level] >= max_num[level]; level++) {
			// 如果当前层的变量达到了 max_num[level]，就将其置为0，然后进入下一层
			loop_vars[level] = 0;
		}
		loop_vars[level]++;  // 对当前层的变量加一
	}
							...
							for (tn[4] = 0; tn[4] < max_num[4]; tn[4]++) {
								for (tn[5] = 0; tn[5] < max_num[5]; tn[5]++) {
									for (tn[6] = 0; tn[6] < max_num[6]; tn[6]++) {
										for (tn[7] = 0; tn[7] < max_num[7]; tn[7]++) {
											for (tn[8] = 0; tn[8] < max_num[8]; tn[8]++) {
												for (tn[9] = 0; tn[9] < max_num[9]; tn[9]++) {
													for (tn[10] = 0; tn[10] < max_num[10]; tn[10]++) {
														for (tn[11] = 0; tn[11] < max_num[11]; tn[11]++) {
															for (tn[12] = 0; tn[12] < max_num[12]; tn[12]++) {
																for (tn[13] = 0; tn[13] < max_num[13]; tn[13]++) {
																	for (tn[14] = 0; tn[14] < max_num[14]; tn[14]++) {
																		for (tn[15] = 0; tn[15] < max_num[15]; tn[15]++) {
																			for (tn[16] = 0; tn[16] < max_num[16]; tn[16]++) {
																				double tm_residue = length;
...
//std::sort(tt_vec.begin(), tt_vec.end(), compareColumn);


//std::cout <<"residue_vec"<<std::endl;

 for (int i= 0; i< residue_vec.size(); i++) {

   std::cout<<residue_vec[i]<<" ";
 }
 std::cout<<endl;
   ...
   if (size!=1) {
	   ...
	   while (level < size) {
		   ...
		   */
		   ...
		   for (level = 0; level < size && nn[level] >= max_vec[level]; level++) {
			   // 如果当前层的变量达到了n，就将其置为min_vec[level]，然后进入下一层
			   nn[level]=min_vec[level];
		   }
		   nn[level]++;
	   }
	   delete[] nn;
...
void cal_lenth_1(){
  ...
  double length = 500.;
  std::vector<double> length_par = { 270.,210.,230.,240.,350. };
  std::vector<int> num_par = { 2,3,4,5,2 };
...
for (int i= 0; i< num_final_vec.size(); i++) {
 ...
 //std::vector<std::vector<int>> tt_vec;
  ...
  std::cout << "//////////tt_vec//////////" << endl;
  for (const auto& row : tt_vec) {
  ...
  */
  ...
  //std::sort(tn_vec.begin(), tn_vec.end(), compareColumn);
  
  std::cout << "//////////tn_vec//////////" << endl;
  for (const auto& row : tn_vec) {
	  for (const auto& element : row) {
		  std::cout << element << " ";
	  }
	  std::cout << std::endl;
...
for (int i= 0; i < length_vec.size(); i++) {
  ...
  if( length_map[i]==0){
	...
	std::cout << std::endl;
    std::cout<<" at least one length can't be founded!!!!!!!!"<<std::endl;
...
int size123 = num_final_vec.size();
std::cout << " num final sizesize = " << size123 << "   "<<endl;
